...
object UnreadFlags extends TableQuery(new UnreadFlags(_)) {
	def contactUnread(user: Int, contact: Rep[Int]) = {
		UnreadFlags.filter(uf => uf.user === user && uf.contact === contact).exists
	}

	def setContactUnread(user: Int, contact: Int)(implicit push: PushService) = {
	...
	def setContactRead(user: Int, contact: Int)(implicit push: PushService) = {
